...
def run_test_environment(test_config):
    ...
    test_config, bottleneck = create_test.create_test_case(rate_range[0], rate_range[1], switch_range[0], switch_range[1], cross_traffic_default)
    ...
    for i in range(iteration):
        ...
        if estimated_available_bandwidth[0] < true_available_bandwidth < estimated_available_bandwidth[1]:
            ...
            absolute_error = min(math.fabs(estimated_available_bandwidth[0] - true_available_bandwidth), math.fabs(estimated_available_bandwidth[1] - true_available_bandwidth))
            relative_error = min(math.fabs(1-estimated_available_bandwidth[0]/true_available_bandwidth),math.fabs(1-estimated_available_bandwidth[1]/true_available_bandwidth))
